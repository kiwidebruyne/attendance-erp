...
| Baseline working beacon day            | `2026-04-03` must keep `emp_001` in `근무 중` with a beacon-authenticated `clockInAt` and no `clockOutAt`.                                               | The baseline day proves a working today card with beacon-auth state instead of a missing-check-in state. |
