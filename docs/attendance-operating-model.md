...
| Admin rejects the manual request  | update `manualAttendanceRequest.status=rejected` and store `reviewComment`  | `activeExceptions` may include `manual_request_rejected`                                                                      | show the review comment and the next action; the canonical attendance fact remains unchanged                    |
