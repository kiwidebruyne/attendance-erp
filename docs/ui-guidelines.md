...
- Top-correction and history CTAs should sync the relevant date or range, update selected-date context, and open that inline composer with the right prefilled flow instead of opening separate modal or sheet owners. When those CTAs open the composer, the page may scroll to the composer area so the write surface is immediately visible.
