...
- On `/attendance/leave`, keep one stable top summary tier visible even when correction candidates exist. That tier should stay calm, lead with leave balance, and summarize the current leave state through four chain counts in this order: `approved`, `pending`, `rejected`, and `revision_requested`.
...
- Keep `/attendance/leave` history as a full-width row above the lower planning workspace so chain actions remain history-led before calendar planning.
