...
- 제품 해석: `감시 도구`가 아니라 `같은 사실을 맞추고 서로를 돕는 도구`, 그리고 사용자 입장에서는 `나를 위한 도구`
...
- 제가 `pnpm`을 선택한 이유는 단순히 "모노레포로 갈 수도 있다" 수준이 아니라, 실제로 모노레포가 되었을 때 `pnpm workspace`, 공유 lockfile, 패키지 경계 관리, workspace 단위 스크립트 실행, 의존성 설치 효율이 자연스럽게 이어지기 때문입니다.
- 나중에 admin 전용 앱이나 공용 UI 패키지, shared contract package 같은 구조로 분리하더라도 `pnpm` 생태계 위에서 큰 마찰 없이 확장할 수 있다고 봤습니다.
- 여기에 `Turborepo` 같은 도구와도 궁합이 좋아서, 훗날 build cache, task pipeline, package graph 기반 실행 최적화까지 연결하기 쉽다는 점도 중요하게 봤습니다.

### Lefthook

- 문서든 코드든 저장소에 들어오는 순간 최소한의 품질 게이트는 자동으로 걸려야 한다고 봤습니다.
- 특히 이 프로젝트는 계약 문서와 구현이 함께 움직이기 때문에, 포맷/린트/테스트가 커밋 흐름에 붙어 있는 편이 안전합니다.
- 그래서 `Husky + lint-staged` 조합보다 `Lefthook`을 택했습니다.
- 이유는 실행 속도 자체도 있지만, 나중에 workspace가 늘어나거나 모노레포로 확장됐을 때 repo-level hook을 한 곳에서 관리하기 더 단순하고, 다중 패키지 구조에서도 운용이 덜 번거롭다고 판단했기 때문입니다.
- 즉 현재 단일 앱 기준으로도 충분히 깔끔하지만, 훗날 `pnpm workspace`나 `Turborepo` 기반 구조가 생겼을 때도 hook 전략을 크게 다시 갈아엎지 않아도 되는 선택이라고 봤습니다.
...
저는 이 프로젝트가 관리자와 직원이 서로를 의심하게 만드는 도구가 아니라, 같은 사실을 보고 빠르게 정렬하는 도구, 서로를 위한 도구, 그리고 각 사용자 입장에서는 결국 `나를 위한 도구`처럼 느껴져야 한다고 생각했습니다.
...
이 방향을 구체적인 문구 톤으로 옮길 때는 Toss의 말투를 많이 참고했습니다.
토스식 말투를 그대로 흉내 내고 싶었다기보다, 딱딱하게 통제하는 말투보다 "지금 무엇이 일어났고, 무엇을 하면 되는지"를 편안하지만 분명하게 말하는 방식이 이 제품에 더 맞는다고 봤습니다.
이 판단은 [docs/ux-writing-guidelines.md](docs/ux-writing-guidelines.md)와 [docs/ui-guidelines.md](docs/ui-guidelines.md)에 직접 반영해 두었습니다.
...
이 관점 때문에 `/attendance`의 exception rail, `/admin/attendance`의 예외 우선 테이블, 문제 있는 항목을 일반 ledger보다 먼저 보여 주는 선택들을 의도적으로 넣었습니다.
이것은 사용자가 꼼꼼히 테이블을 읽어 주기를 기대하는 설계가 아니라, 시스템이 먼저 문제를 발견하고 먼저 드러내서 조용한 오류가 개인의 주의력에만 의존하지 않도록 하려는 설계입니다.
다시 말해 "알아서 잘 찾아보세요"가 아니라, 문제가 생기면 시스템적으로 반드시 눈에 띄게 만들고, 빠르게 처리 경로까지 연결하는 UX를 만들고 싶었습니다.
...
원래는 여기서 한 단계 더 나아가, 사용자가 비콘 범위 밖으로 나갔을 때 `비콘 범위 밖으로 나가셨네요! 혹시 퇴근하셨나요?` 같은 리마인더나 알림을 줘서 퇴근 누락 자체를 사전에 막는 아이디어까지 기획했습니다.
실제 구현까지는 못 갔지만, 이 아이디어 역시 "문제가 생긴 뒤에만 고치는 시스템"이 아니라 "누락이 생기기 전에 먼저 막는 시스템"을 만들고 싶었다는 연장선에 있습니다.
...
다만 제품 아이디어 차원에서는, 비콘 범위 밖 상황을 단순 조회 상태로만 끝내지 않고 퇴근 누락을 예방하는 리마인더 트리거로도 활용할 수 있다고 봤습니다.
예를 들면 `비콘 범위 밖으로 나가셨네요! 혹시 퇴근하셨나요?`처럼 부드럽지만 분명한 알림을 주어, 사용자가 퇴근 기록을 빠뜨리기 전에 한 번 더 확인하게 만드는 흐름을 기획했습니다.
이 부분은 현재 구현에는 포함되지 않았지만, 나중에 알림/리마인더를 확장할 때 가장 먼저 붙이고 싶은 아이디어 중 하나였습니다.
...
- 관리자가 뒤에서 조용히 승인을 되돌리면
- 직원은 그 사실을 인지하지 못한 채 여전히 승인된 상태라고 믿고 행동할 수 있고
- 그 순간 실제 운영과 사용자 인식 사이에 위험한 차이가 생깁니다
- 이후에 양쪽 화면 sync 문제까지 따라붙을 수 있고, 결과적으로 신뢰가 크게 무너집니다
...
## 12. 과제를 진행하며 정리한 개발 노하우

이 과제는 단순히 화면과 API를 만드는 작업이 아니라, `AI에게 어떻게 맥락을 주고`, `어떻게 검증시키고`, `어떻게 협업해야 성능이 가장 잘 나오는가`를 많이 체감한 작업이기도 했습니다.
아래 내용은 제가 이번 과제를 하면서 정리한 개인적인 개발 노하우입니다.

### 1. Codex는 문서와 지시 품질에 매우 민감했고, Claude Code는 빠른 초안에 강하다고 느꼈습니다

제가 체감한 Codex의 가장 큰 특징은 지시사항을 매우 엄격하게 따르고, 프롬프트에 적힌 내용을 그대로 이행하려는 성향이 강하다는 점이었습니다.
그래서 `AGENTS.md`, skill, `docs/` 문서를 얼마나 정교하게 깎아 두느냐에 따라 결과물 품질 차이가 매우 크게 났습니다.
문서와 규칙이 정교할수록 사용자의 의도에서 벗어나는 오류가 줄고, 정말 원하는 방향대로 사용할 수 있었습니다.

반면 Claude Code는 주어진 맥락을 파악하고 눈치껏 잘 처리하려는 경향이 더 강하다고 느꼈습니다.
빠른 초안 작성이나, 구체적 지시 없이도 일정 수준의 결과물을 빨리 내는 데에는 강점이 있었지만, 그만큼 가끔은 사용자의 정확한 의도에서 살짝 벗어나는 경우도 있었습니다.

### 2. Plan Mode는 거의 항상 켜 두는 편이 좋다고 느꼈습니다

제가 경험한 바로는 Plan Mode를 켠 상태에서 AI가 먼저 구조를 잡게 두는 편이 전체 품질이 더 좋았습니다.
특히 agent가 세운 plan을 사람이 중간에서 과도하게 뜯어고치기 시작하면, plan의 일관성도 깨지고 최종 구현 품질도 함께 떨어지는 경우가 많았습니다.
즉, plan은 가볍게 방향을 맞추는 데 쓰고, 한 번 합의된 뒤에는 지나치게 흔들지 않는 편이 결과가 더 좋았습니다.

### 3. AI는 처음부터 다시 시켜도 되기 때문에, 때로는 수정보다 재시작이 더 효율적입니다

사람에게 같은 일을 처음부터 다시 해 달라고 하면 비용이 크지만, AI는 그 점에서 훨씬 유연합니다.
그래서 작업물이 애매하게 마음에 안 들 때는 기존 결과를 억지로 수정하려고 끌고 가기보다, 차라리 처음부터 다시 시키는 편이 더 빠르고 깔끔할 때가 있었습니다.
이 점은 특히 초안의 방향 자체가 틀렸을 때 더 크게 체감했습니다.

### 4. 이제 사람은 병목이 되기 쉽다고 느꼈습니다

이번 과제를 하면서 오히려 가장 느린 부분은 구현 그 자체보다 사람의 판단과 개입이었습니다.
그래서 사람이 매 단계마다 세부 구현에 직접 개입하기보다, 가능한 한 워크플로우에서 사람의 반복 작업을 줄이고 승인과 검증에 집중하는 편이 더 빠르고 안정적이라고 느꼈습니다.
즉, 사람이 모든 것을 직접 만드는 시대라기보다, 사람이 방향을 정하고 AI가 밀어 붙인 뒤 사람이 승인과 검증을 맡는 구조가 훨씬 효율적이었습니다.

### 5. 사소한 명명과 구조 선택은 오히려 AI에게 맡기는 편이 더 낫다고 느꼈습니다

파일명, 변수명, 함수명, 폴더 구조처럼 자잘하지만 반복적으로 판단해야 하는 것들은 사람이 오래 고민할수록 오히려 비효율적일 때가 많았습니다.
이런 부분은 AI가 훨씬 많은 학습 데이터 위에서 더 직관적이고 평균적으로 무난한 선택을 내릴 가능성이 높다고 느꼈습니다.
그래서 저는 작은 naming이나 구조 선택은 AI에게 더 많이 맡기고, 사람이 정말 개입해야 하는 판단은 제품 방향이나 정책 같은 본질적인 문제에 집중하는 편이 낫다고 봤습니다.

### 6. 리뷰는 반드시 다른 agent나 완전히 다른 세션에게 시켜야 한다고 느꼈습니다

본인이 만든 결과물을 본인에게 다시 리뷰시키면, critical한 문제를 놓치는 경우가 생각보다 많았습니다.
같은 맥락과 같은 사고 흐름 안에 있기 때문에, 작성 당시의 가정을 그대로 유지한 채 보는 경향이 강했기 때문입니다.
그래서 리뷰는 가능하면 subagent나 완전히 분리된 세션에 맡기는 편이 훨씬 낫다고 느꼈습니다.
이 과제를 하면서도 `구현 agent`와 `리뷰 agent`를 분리할수록 더 객관적인 지적이 나온다는 점을 체감했습니다.

### 7. 문서 관리 품질이 곧 AI 성능이라고 느꼈습니다

AI는 결국 주어진 맥락 안에서만 잘할 수 있습니다.
그래서 `docs/` 문서를 기록하면서 작업하는 것이 단순한 정리 습관이 아니라, AI에게 구현 방향을 설명하는 핵심 수단이 되었습니다.
문서가 좋아질수록 AI의 판단과 구현도 더 안정적이었고, 반대로 문서가 빈약하면 프롬프트를 아무리 길게 써도 품질이 흔들리기 쉬웠습니다.

### 8. 기록이 없으면 맥락 제공 자체가 불가능하다고 느꼈습니다

이 점은 GitHub issue와 PR을 쓰면서 특히 더 명확하게 느꼈습니다.
기록이 없으면 나중에 AI에게 맥락을 전달하고 싶어도 전달할 수가 없습니다.
그래서 issue, PR, commit history, 문서 모두를 가능한 한 구체적으로 남겨야 했습니다.
결국 AI 협업에서 기록은 부가 업무가 아니라, 나중에 다시 맥락을 재구성하고 이어서 작업하기 위한 필수 인프라에 가깝다고 느꼈습니다.

### 9. 모든 변경을 TDD로 끌고 가는 것은 이번 과제에서는 비효율적이라고 느꼈습니다

저는 이번 과제에서 TDD를 모든 작업에 기계적으로 적용하는 방식은 속도를 너무 많이 늦추고, 토큰 소비량도 크게 늘린다고 느꼈습니다.
수정사항이 자주 바뀌고 문서와 제품 방향이 계속 다듬어지는 상황에서는, 변경마다 테스트 코드를 먼저 쓰는 방식이 오히려 낭비가 될 때도 많았습니다.
그래서 저는 `무조건적인 TDD`보다, 문서와 계약으로 방향을 먼저 고정하고, 위험한 로직과 사용자 상호작용에 대해 필요한 테스트를 선택적으로 두는 편이 더 현실적이라고 판단했습니다.
